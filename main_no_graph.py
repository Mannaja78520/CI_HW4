...
def load_xlsx_first_sheet_numeric(path):
    with zipfile.ZipFile(path, 'r') as z:
        ...
        for r in range(max_r+1):
            row = []
            for c in range(15):  # 0..14 (ตามคำสั่งโจทย์)
                v = data.get((r,c), None)
                if c in (0,1):
...
# ---------- “No-Plot” Presenter ----------
def print_arch_comparison(arch_list, s5, s10):
    labels = ["-".join(map(str,a)) for a in arch_list]
    print("\n=== MAE(test) by Architecture (Days) ===")
    w = max(12, max(len(lb) for lb in labels))
    print(f"{'Architecture'.ljust(w)}  {'5 days':>12}  {'10 days':>12}")
    for lb, m5, m10 in zip(labels, s5, s10):
        f5  = f"{m5:.4f}"  if np.isfinite(m5)  else "nan"
        f10 = f"{m10:.4f}" if np.isfinite(m10) else "nan"
        print(f"{lb.ljust(w)}  {f5:>12}  {f10:>12}")
...
def main():
    ...
    print_arch_comparison(ARCHS, s5, s10)
